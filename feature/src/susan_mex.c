/* MEXified (and worked) version of this SUSAN code by:
	Joaquim Luis:	jluis@ualg.pt

	To get an fast help, just run the program name without any arguments, e.g.:
...
usage() {
	mexPrintf("Usage: susan(gray, [options])\n\n");

	mexPrintf("-e : Edges mode - output a mask (logical) edges image [default]\n");
	mexPrintf("-ei: Edges Image mode (overlay edges on input image)\n");
	mexPrintf("-s : Smoothing mode\n");
	mexPrintf("-c : Corners mode - output a Mx2 corners matrix with (image x,y corners coordinates)\n\n");
	mexPrintf("-ci: Corners Image mode (overlay corners on input image)\n\n");
	mexPrintf("See source code for more information about setting the thresholds\n");
	mexPrintf("-t <thresh> : Brightness threshold, all modes (default=20)\n");
	mexPrintf("-d <thresh> : Distance threshold, smoothing mode, (default=4) (use next option instead for flat 3x3 mask)\n");
	mexPrintf("-3 : Use flat 3x3 mask, edges or smoothing mode\n");
	mexPrintf("-n : No post-processing on the binary edge map (runs much faster); edges mode\n");
	mexPrintf("-q : Use faster (and usually stabler) corner mode; edge-like corner suppression not carried out; corners mode\n");
	mexPrintf("-b : Mark corners/edges with single black points instead of black with white border; corners or edges mode\n");
	mexPrintf("-p : Output initial enhancement image only; corners or edges mode (default is edges mode)\n");
	mexPrintf("SUSAN Version 2l (C) 1995-1997 Stephen Smith, DRA UK. steve@fmrib.ox.ac.uk\n\n");
	mexPrintf("But this is its MEXified version by J. Luis, UALG PT. jluis@ualg.pt\n");
}


int_to_uchar(int *r, unsigned char *in, int size) {
	int i,
	max_r=r[0],
	min_r=r[0];
...
susan_principle(unsigned char *in,int *r,unsigned char *bp,int max_no,int x_size,int y_size) {
	int   i, j, n;
	uchar *p,*cp;
...
susan_principle_small(unsigned char *in, int *r, unsigned char *bp, int max_no, int x_size, int y_size) {
	int   i, j, n;
	uchar *p,*cp;
...
uchar median(unsigned char *in, int i, int j, int x_size) {
	int p[8],k,l,tmp;
...
void susan_smoothing(int three_by_three,uchar *in,float dt,int x_size,int y_size,uchar *bp) {
	...
	if ( (dt > 15) && (total == 0) ) {
		mexPrintf("Distance_thresh (%f) too big for integer arithmetic.\n",dt);
		mexPrintf("Either reduce it to <=15 or recompile with variable \"total\"\n");
		mexPrintf("as a float: see top \"defines\" section.\n");
		mexErrMsgTxt("");
	}

	if ( (2*mask_size+1>x_size) || (2*mask_size+1>y_size) ) {
		mexPrintf("Mask size (1.5*distance_thresh+1=%d) too big for image (%dx%d).\n",mask_size,x_size,y_size);
		mexErrMsgTxt("");
	}
...
edge_draw(unsigned char *in, unsigned char *mid, int x_size, int y_size, int drawing_mode, int outImage) {
	int   i;
	...
	midp = mid;
	if (outImage) {		/* Output is the image with the detected edges */
		for (i = 0; i < x_size * y_size; i++) {
			if (*midp < 8) 
				*(in + (midp - mid)) = 0;
			midp++;
	...
	else {		/* Output will only contain the detected edges */
		for (i = 0; i < x_size * y_size; i++) {
			if (*midp < 8)
				*(in + (midp - mid)) = 1;
			else
...
susan_thin(int *r, unsigned char *mid, int x_size, int y_size) {
	int	l[9], centre, b01, b12, b21, b10, p1, p2, p3, p4,
		b00, b02, b20, b22, m, n, a, b, x, y, i, j;
	uchar	*mp;

  for (i=4;i<y_size-4;i++)
    for (j=4;j<x_size-4;j++)
      if (mid[i*x_size+j]<8) {
        centre = r[i*x_size+j]; /* count number of neighbours */
...
susan_edges( unsigned char *in, int *r, unsigned char *mid, unsigned char *bp, int max_no, int x_size, int y_size) {
	float z;
...
susan_edges_small(unsigned char *in, int *r, unsigned char *mid, unsigned char *bp, int max_no, int x_size, int y_size) {
	float z;
	int   do_symmetry, i, j, m, n, a, b, x, y, w;
	unsigned char c,*p,*cp;
...
corner_draw(unsigned char *in,CORNER_LIST corner_list, int x_size, int drawing_mode) {
	uchar *p;
	int   n=0;
...
int susan_corners(uchar *in, int *r, uchar *bp, int max_no, CORNER_LIST corner_list, int x_size, int y_size) {
...
#endif
{
				...
				n++;
				if (n == MAX_CORNERS) {
					mexPrintf("Number of currently available corners exceeded (within this version).\n");
					mexPrintf("If you really want more than = %d corners, you must recompile me.\n", MAX_CORNERS);
					goto L20;
				}
			}
		}
...
int susan_corners_quick(uchar *in, int *r, uchar *bp, int max_no, CORNER_LIST corner_list, int x_size, int y_size) {
	int   n,x,y,i,j;
	uchar *p,*cp;
...
#endif
{
				...
				x = in[(i-2)*x_size+j-2] + in[(i-2)*x_size+j-1] + in[(i-2)*x_size+j] + 
					in[(i-2)*x_size+j+1] + in[(i-2)*x_size+j+2] + in[(i-1)*x_size+j-2] + 
					in[(i-1)*x_size+j-1] + in[(i-1)*x_size+j] + in[(i-1)*x_size+j+1] + 
					in[(i-1)*x_size+j+2] + in[(i  )*x_size+j-2] + in[(i  )*x_size+j-1] + 
					in[(i  )*x_size+j] + in[(i  )*x_size+j+1] + in[(i  )*x_size+j+2] + 
					in[(i+1)*x_size+j-2] + in[(i+1)*x_size+j-1] + in[(i+1)*x_size+j] + 
					in[(i+1)*x_size+j+1] + in[(i+1)*x_size+j+2] + in[(i+2)*x_size+j-2] + 
					in[(i+2)*x_size+j-1] + in[(i+2)*x_size+j] + in[(i+2)*x_size+j+1] + 
					in[(i+2)*x_size+j+2];

				corner_list[n].I=x/25;
				/*corner_list[n].I=in[i*x_size+j];*/
				x = in[(i-2)*x_size+j+2] + in[(i-1)*x_size+j+2] + in[(i)*x_size+j+2] + 
					in[(i+1)*x_size+j+2] + in[(i+2)*x_size+j+2] - (in[(i-2)*x_size+j-2] + 
					in[(i-1)*x_size+j-2] + in[(i)*x_size+j-2] + in[(i+1)*x_size+j-2] + 
					in[(i+2)*x_size+j-2]);
				x += x + in[(i-2)*x_size+j+1] + in[(i-1)*x_size+j+1] + in[(i)*x_size+j+1] + 
					in[(i+1)*x_size+j+1] + in[(i+2)*x_size+j+1] - (in[(i-2)*x_size+j-1] + 
					in[(i-1)*x_size+j-1] + in[(i)*x_size+j-1] + in[(i+1)*x_size+j-1] + 
					in[(i+2)*x_size+j-1]);

				y = in[(i+2)*x_size+j-2] + in[(i+2)*x_size+j-1] + in[(i+2)*x_size+j] + 
					in[(i+2)*x_size+j+1] + in[(i+2)*x_size+j+2] - (in[(i-2)*x_size+j-2] + 
					in[(i-2)*x_size+j-1] + in[(i-2)*x_size+j] + in[(i-2)*x_size+j+1] + 
					in[(i-2)*x_size+j+2]);
				y += y + in[(i+1)*x_size+j-2] + in[(i+1)*x_size+j-1] + in[(i+1)*x_size+j] + 
					in[(i+1)*x_size+j+1] + in[(i+1)*x_size+j+2] - (in[(i-1)*x_size+j-2] + 
					in[(i-1)*x_size+j-1] + in[(i-1)*x_size+j] + in[(i-1)*x_size+j+1] + 
					in[(i-1)*x_size+j+2]);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	char   filename [80], *tcp, **argv;
	unsigned char  *in, *bp, *mid, *in_data, *outByte;
	int	c, n, m, nn, nCorners = 0, *nVector, *mVector, outImage = 0, outCornos = 0;
	int	argc = 0, n_arg_no_char = 0, error = 0;
	int	*r, argindex = 3, bt = 20, principle = 0, thin_post_proc = 1, three_by_three = 0,
		drawing_mode = 1, susan_quick = 0, max_no_corners = 1850, max_no_edges = 2650,
		mode = 1, i, x_size, y_size;
	float	dt = 4.0;
	double	*cornos;
	double *input;
	...
	for (i = 1; !error && i < argc; i++) {
		if (argv[i][0] == '-') {
			switch (argv[i][1]) {
				...
				case 's':	/* smoothing */
					mode = 0;
					outImage = 1;	/* it must be */
					break;
				case 'e':	/* edge [default]*/
					mode = 1;
					if (argv[i][2])		/* edge on image */
						outImage = 1;
					break;
				case 'c':	/* corners */
					mode = 2;
					outImage = 0;
					outCornos = 1;
					if (argv[i][2]) {	/* output corners over input image */
						outImage = 1;
						outCornos = 0;
					}
					break;
			...
			mexPrintf("Error using options. Missing the leading '-'\n");
	}
	mxFree(argv);
	...
	if (mxGetNumberOfDimensions(prhs[0]) != 2)
		mexErrMsgTxt("Sorry, I'm very Daltonic and therefore I only accept gray scale images\n");

	if (drawing_mode == 0) {
		if (outCornos || !outImage) {
			mexPrintf("There was a conflict with the selected options. Marking corner/edges\n");
			mexPrintf("with a white border, Won. Corolary, output is always an image.\n");
		}
		outImage = 1;
		outCornos = 0;
	}

	y_size = mxGetM(prhs[0]);
	x_size = mxGetN(prhs[0]);
	// in_data = (unsigned char *)mxGetData(prhs[0]);
	input = mxGetPr(prhs[0]);;
    in = (unsigned char *)mxCalloc (x_size*y_size, sizeof (unsigned char));

	nVector = mxCalloc(x_size, sizeof(int));
	mVector = mxCalloc(y_size, sizeof(int));
	for (n = 0; n < x_size; n++) nVector[n] = n*y_size-1 + y_size;
	for (m = 0; m < y_size; m++) mVector[m] = m*x_size;

	for (m = 0; m < y_size; m++) for (n = 0; n < x_size; n++) {
		nn = nVector[n]-m;
		in[mVector[m]+n] = (unsigned char)input[nn]; // in_data[nn];
	}
	...
	switch (mode) {
		...
		case 1:	/* edges */
			r = mxCalloc(x_size * y_size, sizeof(int));
			...
			if (principle) {
				...
				mid = mxCalloc(x_size * y_size, sizeof(char));
				...
				edge_draw(in,mid,x_size,y_size,drawing_mode, outImage);
				mxFree(mid);
			}
			mxFree(r);
			break;
		...
		case 2:	/* corners */
			...
			if (principle) {
				...
				if (susan_quick)
					nCorners = susan_corners_quick(in,r,bp,max_no_corners,corner_list,x_size,y_size);
				else
					nCorners = susan_corners(in,r,bp,max_no_corners,corner_list,x_size,y_size);
				if (outImage)		/* only at an ouput image request will we draw corners */
	...
	if (outCornos)		/* Ouput is a corners array */
		plhs[0] = mxCreateNumericMatrix (nCorners,2,mxDOUBLE_CLASS, mxREAL);
	else if (outImage)
		plhs[0] = mxCreateNumericMatrix (y_size,x_size,mxUINT8_CLASS, mxREAL);
	else
		plhs[0] = mxCreateNumericMatrix (y_size,x_size,mxLOGICAL_CLASS, mxREAL);


	if (!outCornos) {
		outByte = (unsigned char *)mxGetData(plhs[0]);
		for (m = y_size-1, c = 0; m >= 0; m--) for (n = 0; n < x_size; n++)
			outByte[m + n*y_size] = in[c++];
	}
	else {
		cornos = (double *)mxGetData(plhs[0]);
		for (i = 0; i < nCorners; i++) {
			cornos[i] = corner_list[i].x;
			cornos[i+nCorners] = corner_list[i].y;
		}
	}
