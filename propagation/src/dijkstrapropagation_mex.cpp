/*
	[D,S] = dijkstrapropagation_mex(W,start_verts,end_verts,nb_iter_max,H);
*/
...
void check_heap( int i ) {
	for( int x=0; x<n; ++x ) {
			if( heap_pool[x]!=NULL ) {
				int j = (intptr_t) heap_pool[x]->fhe_data;
				if( H==NULL ) {
					if( D[i]>D[j] )
...
//printf("\n\nperform the front propagation (y or no): %d", !fh_isempty(open_heap));
	...
	while( !fh_isempty(open_heap) && num_iter<nb_iter_max && !stop_iteration )
	{
		...
		// current point
		int i = (intptr_t) fh_extractmin( open_heap );
		heap_pool[i] = NULL;
		S[i] = kDead;
        ...
        /* index in   irs[jcs[i]]...irs[jcs[i+1]-1] are the node connected
         * to node i
         * values are W[jcs[i]]...W[jcs[i]-1]
         * recurse on each neighbor of i */
        for( int k=jcs[i]; k<jcs[i+1]; ++k ) {
            int ii = irs[k];
            double P = W[k]; // graph weight
            
            if(incremental) P = P - W[ii];  // incremental version
		...
		/*			// check if action has change. Should not happen for Dijkstra
         ...
         happens for multiple sources Dijkstra!!!!
         */
#if 1
...
{ 
	/* retrieve arguments */
	if( nrhs<4 ) 
		mexErrMsgTxt("4, 5 or 6 input arguments are required."); 
	if( nlhs<1 ) 
    ...
    if (end_points[0]<0) {
        end_points = NULL;
        nb_end_points =0;
    } else nb_end_points = mxGetN(prhs[2]);
        
    // fourth argument : nb_iter_max
    nb_iter_max = (int) *mxGetPr(prhs[3]);

    // fifth argument : heuristic
    if( nrhs>=5 )	{
        H = mxGetPr(prhs[4]);
        ...
        if (nrhs==6) {
            if( !mxIsLogicalScalar(prhs[5])) {
                mexErrMsgTxt("Input incremental must be a logical scalar.");
            } else
                incremental = (int)(mxGetScalar(prhs[5]));
        } else 
            incremental = (int)0;
    } else
        H = NULL;
