// FastMarching2D.cpp : Defines the entry point for the application.
//
...
//================================================================
{
    ...
    for( int s=0; s<nb_start_points; ++s ){
       /* we changed it for compatibility with windows */
        i = floor(start_points[2*s] + 0.5);
		j = floor(start_points[1+2*s]);
        /*		
        i = round(start_points[2*s] + 0.5);
