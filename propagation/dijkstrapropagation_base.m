function [D,P] = dijkstrapropagation_base(W, start_verts, end_verts, niter)
% DIJKSTRAPROPAGATION_BASE - Shortest distance from multiple source points
% on graph.
%
%   [D,P] = DIJKSTRAPROPAGATION_BASE(W, start_verts, end_verts, niter);
%
% Outputs:
...
%
% See reference: 
%  http://www.comp.nus.edu.sg/~xujia/mirror/algorithm.myrice.com/resources/
%          technical_artile/fibonacci_heap/fibonacci.htm 
% and source code available at      http://web.mit.edu/cocosci/isomap/code/
%
% See also 
% Related: DIJKSTRA, DIJKSTRA_BASE, DIJK_BASE, DIJKSTRADVANCED_BASE --
% Called: DIJKSTRAPROPAGATION_MEX.

n = size(W,1); % n = length(W);  % number of points in the graph
m = length(start_verts);

if sum(start_verts>n)>0 || sum(end_verts>n)>0
    error('dijkstrapropagation_base:inputerror', ...
...
if isequal(start_verts,end_verts)
    ...
    if exist('dijkstraset_mex','file')
        ...
        for i=1:m
            [d, p] = dijkstrapropagation_base(W, start_verts(i), end_verts, Inf);
            D(i,:) = d(:)';
            P(i,:) = p(:)';
        end
...
% DIJKSTRASLOWPROPAGATION_BASE - Another Matlab implementation of Dijkstra
% algorithm.
