%% IM2POTENTIAL - Build a potential function from an image.
%
%% Description
% Given an input image, design a metric to be used as a potential (cost)
% function in front propagation over its domain. Typically, we extract norm 
% and directional information from its derivatives to build the metric. In
% particular, (iso- or anisotropic) metrics can be designed from the gradient
% or the gradient structure tensor of the input image.
%
%% Syntax
%   P = IM2POTENTIAL(I, method);
%   P = IM2POTENTIAL(I, method, a, rho, sigma, der, int, samp, eign);
% 
%   P = IM2POTENTIAL(I, 'uni', a);
%   P = IM2POTENTIAL(I, 'pix'/'pixinv', a);
%   P = IM2POTENTIAL(I, 'grd*'/'iso', a, sigma);
%
%   P = IM2POTENTIAL(I, 'gst*'/'iso'/'ani', a, rho, sigma, der, int, samp, eign);
%   P = IM2POTENTIAL(I, 'gst*'/'iso'/'ani', a, rho, sigma, der, int, samp, N);
%   P = IM2POTENTIAL(T, 'gst*'/'iso'/'ani', a, eign);
%   P = IM2POTENTIAL(T, 'gst*'/'iso'/'ani', a, N);   
% 
%% Inputs
...
%          (note that the higher the value at a pixel, the higher its
%          potential, the faster the propagation through it),
% * |'pixinv'|, then the potential is scalar and equal to the inverse of the 
%          image (inversely: the higher the value at a pixel, the lower its
...
% * |'grdn'| (or |'isoinv'|), then the potential is scalar and equal to the
%          gradient norm,
% * |'grdninv'| (or |'iso'|), then the potential is scalar and equal to the
...
%
% * |'iso'| (or |'gstninv'|) for a scalar potential set to the inverse of the
%          GST norm,
% * |'gst'| for a tensorial potential set to the GST itself (same eigen-
%          decomposition),
% * |'gstorth'| for a tensorial potential set to the orthogonal tensor to the
%          GST (same eigenvalues, orthogonal eigenvectors),
...
%          scaled by the coherence (see |GSTFEATURE|),
% * |'gstiso'| for a unitary tensorial potential set to the normalized GST
%          (same eigenvectors, normalized eigenvalues),
%
...
%     one among (see |GRDSMOOTH|): |'matlab'|, |'vista'|, |'fast'|, |'conv'|,
...
%     method used for its approximation; it is either (see |GSTFEATURE|): 
%     |'l1'| (or |'zen'|), |'abs'|, |'sum'| (or |'sap'|), |'dif'| (or |'koe'|)
%     or |'ndi'|; default: |eign='l1'|. 
%
% *|N|* : additional scaling (or pilot) function; it is a matrix of size |(X,Y)|
%     used to scalenormalize the eigenvalues of the GST when the potential is
%     derived from it; it counter-balances the influence of the eigenvalues
%     in the potential definition; typically, it is set to the input image,
%     or the gradient norm; default |N = []|, and the gradient norm is
%     estimated and used as the scale function.
% 
%% Outputs
% *|P|* : potential function derived from the image (see also function 
%     |POTENTIAL2FRONT|); it can be:
%
% * a vector of size |(n,m,2)| representing a vector field used to define an
%          anisotropic metric when the image is scalar
% * a matrix of size |(n,m)| representing a scalar potential field; it will
%          be used for defining an isotropic metric,
% * a tensor matrix of size |(n,m)| representing a tensor field when the
%          image is multispectral, and used for defining an anisotropic
%          Riemannian metric.
%
%% References
...
elseif C==1 && any(strcmpi(method(1:3),{'grd','iso'}))
    ...
    % im2potential(X,'grdninv',a) is equivalent, but calling
    % im2potential(X,'grdninv',a, rho) is not.
    % similarly, it is equiavelent to call im2potential(X,'pix',a)
    % or im2potential(X,'grdn',a), but not the call to
...
elseif any(strcmpi(method(1:3),{'gst','iso','ani'})) % the input can be any dimension
    ...
    if any(strcmpi(method,{'iso','gstninv','gstn'}))
        switch method
            case {'iso','gstninv'}
                % we return the inverse of the tensor norm for isotropic
                % propagation: the higher the gradient norm value of a pixel,
                % the lower its potential, the slower the propagation through
                % it
                P = 1 ./ (cons + N).^a(1);
    ...
    error('im2potential:methoderror', ...
