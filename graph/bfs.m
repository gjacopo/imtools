...
%
%% Description
% Compute breadth first search (BFS) distances, times, and tree for a graph
% and enumerates valid paths.
%
%% Syntax
%     [d dt path pred] = bfs(A,u,target);
%
% |[d dt pred] = bfs(A,u)| returns the distance |d| and the discover time
% |dt| for each vertex in the graph in a breadth first search starting from 
% vertex |u|.
%
...
%
% * |pred| is the predecessor array, |pred(i)=0| if vertex |i| is in a component  
% not reachable from |u| and |i != u|.
%
%% Example
...
%% See also
% Related:
% <matlab:webpub(whichpath('DFS')) |DFS|>,
